    ...
    return fetch(url)
        ...
        .then(function(html) {
            ...
            $("a[href*='/anime/info/']").each(function(i, el) {
                ...
                var title = $(el).find("h3, .title, figcaption, [class*='title']").text().trim();

                if (!title) title = $(el).attr("title") || "";

                if (href && title) {
                    var idMatch = href.match(/\/anime\/info\/([^?#]+)/);
...
Provider.prototype.findEpisodeServer = function(episode, serverId) {
    ...
    var animeId = parts[0];
    var epNum = episode.number || parts[1] || 1;

    // Placeholder using the m3u8 you found
    return Promise.resolve({
